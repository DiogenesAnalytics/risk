...
class TrueskillRanker(object):
    ...
    def update(self, winner_pids, loser_pids):
        ...
        new_winner_ratings, new_loser_ratings = self.ts.rate(
            [winner_ratings, loser_ratings], [0, 1]
        )
    ...
    def rank(self):
        ...
        """
        return sorted(
            ((pid, self.score(pid)) for pid in self.ratings.keys()),
            key=lambda x: x[1],
            reverse=True,
        )
...
class RiskRanker(TrueskillRanker):
    ...
    def player_pools(self):
        ...
        return itertools.izip(
            *[
                itertools.islice(ids, i, None, self.n_players)
