...
class BaseMission(object):
    ...
    def __repr__(self):
        return 'Mission("{desc}", {p})'.format(
            ...
            p="assigned to " + definitions.player_colors[self.player_id]
    ...
    def _score(self, board):
        return max(0.0, board.n_territories(self.player_id) / 24.0)
...
class TerritoryMission(BaseMission):
    ...
    def _criterium(self, board):
        ...
        return len(
            [
                tid
                for (tid, pid, arm) in board.data
                if (pid == self.player_id and arm >= 2)
            ]
        )
    ...
    def _score(self, board):
        ...
        return self._criterium(board) / 18.0
...
class PlayerMission(BaseMission):
    ...
    def description(self):
        if self.target_id == self.player_id:
            ...
            return "eliminate the {color} player".format(
                color=definitions.player_colors[self.target_id]
            )
    ...
    def _score(self, board):
        if self.target_id == self.player_id:
            ...
            return (42 - (board.n_territories(self.target_id))) / 42.0
...
class ContinentMission(BaseMission):
    ...
    def description(self):
        return "conquer {continents}".format(
            continents=" and ".join(
                [definitions.continent_names[cid] for cid in self.continents]
            )
        )
...
class ExtraContinentMission(ContinentMission):
    ...
    def _score(self, board):
        ...
        add_score = max(
            (
                board.continent_fraction(cid, self.player_id)
                for cid in self.other_continents
            )
        )
...
def missions(n_players):
    ...
    player_missions = [PlayerMission(pid) for pid in range(n_players)]
