...
class Board(object):
    ...
    def create(cls, n_players):
        ...
        return cls(
            [
                Territory(territory_id=tid, player_id=pid, armies=1)
    ...
    def hostile_neighbors(self, territory_id):
        ...
        return (
            t
            for t in self.data
            if (t.player_id != player_id and t.territory_id in neighbor_ids)
        )
    ...
    def continent(self, continent_id):
        ...
        return (
            t
            for t in self.data
            if t.territory_id in definitions.continent_territories[continent_id]
        )
    ...
    def num_foreign_continent_territories(self, continent_id, player_id):
        ...
        """
        return sum(
            1 if t.player_id != player_id else 0 for t in self.continent(continent_id)
        )
    ...
    def fortify(self, from_territory, to_territory, n_armies):
        ...
        if n_armies < 0 or self.armies(from_territory) <= n_armies:
            raise ValueError(
                "Board: Cannot move {n} armies from territory {tid}.".format(
                    n=n_armies, tid=from_territory
                )
            )
        if to_territory not in [
            t.territory_id for t in self.friendly_neighbors(from_territory)
        ]:
            raise ValueError(
                "Board: Cannot fortify, territories do not share owner and/or border."
            )
    ...
    def attack(self, from_territory, to_territory, attackers):
        ...
        if attackers < 1 or self.armies(from_territory) <= attackers:
            raise ValueError(
                "Board: Cannot attack with {n} armies from territory {tid}.".format(
                    n=attackers, tid=from_territory
                )
            )
        if to_territory not in [
            tid for (tid, _, _) in self.hostile_neighbors(from_territory)
        ]:
            raise ValueError(
                "Board: Cannot attack, territories do not share border or are owned by the same player."
            )
    ...
    def plot_board(self):
        ...
        im = plt.imread(os.path.join(assets_dir, "risk.png"))
    ...
    def plot_single(territory_id, player_id, armies):
        ...
        plt.scatter(
            ...
            [coor[1] * 1.22],
            s=1250,
            c=definitions.player_colors[player_id],
        )
        plt.text(
            coor[0] * 1.2,
            coor[1] * 1.22 + 25,
            s=str(armies),
            color="black"
            if definitions.player_colors[player_id] in ["yellow", "pink"]
            else "white",
            ha="center",
            size=30,
        )
    ...
    def fight(cls, attackers, defenders):
        ...
        attack_dices = sorted(
            [cls.throw_dice() for _ in range(n_attack_dices)], reverse=True
        )
        defend_dices = sorted(
            [cls.throw_dice() for _ in range(n_defend_dices)], reverse=True
        )
    ...
    def set_owner(self, territory_id, player_id):
        ...
        Args:
            ...
            player_id (int): ID of the player.
        """
        self.data[territory_id] = Territory(
            territory_id, player_id, self.armies(territory_id)
        )
    ...
    def set_armies(self, territory_id, n):
        ...
        if n < 1:
            raise ValueError(
                "Board: cannot set the number of armies to <1 ({tid}, {n}).".format(
                    tid=territory_id, n=n
                )
            )
