...
class GeneticPlayer(Genome, SmartPlayer):
    ...
    def min_attack_weight(self, won_yet):
        ...
        """
        return self["att_cutoff"] + (self["att_cutoff_win"] if not won_yet else 0.0)
    ...
    def fortify(self):
        ...
        if len(possible_fortifications) == 0:
            return None
        fortification = max(
            possible_fortifications, key=lambda x: self.fortification_weight(x)
        )
    ...
    def fortification_weight(self, fortification):
        return sum(
            (
                ...
                (self.territory_vantage_difference(fortification))
                * self["ft_tvantage_wgt"],
                (
                    self.mission_value(fortification.from_territory_id)
                    - self.mission_value(fortification.to_territory_id)
                )
                * self["ft_mission_wgt"],
                (
                    self.direct_bonus(fortification.from_territory_id)
                    - self.direct_bonus(fortification.to_territory_id)
                )
                * self["ft_bonus_wgt"],
    ...
    def reinforce(self):
        ...
        if isinstance(self.mission, TerritoryMission) and len(options) >= 18:
            ...
            if (
                len(options) == 0
            ):  # in this case the player has in principle won, and only needs to finish his turn
                options = self.territories
